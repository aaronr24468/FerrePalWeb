    ...
    return (
        <dialog id="info_credit" >
            ...
            {selectModal === "Info" && <div className="info_credit_Container">
                ...
                <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Monto total </span>{credit.amount}</p></div>
                <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Saldo pendiente </span>{(Number(credit.amount) - Number(credit.Installment)).toFixed(2)}</p></div>
                <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Total abonado </span>{credit.Installment}</p></div>
            ...
            {selectModal === "Edit" && <div className="info_credit_Container">
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="info_credit_customer_edit"><span className="amount_edit">Monto total</span> <span className="current_amount">{credit.amount}<input type="text" defaultValue={'+0.00'} className="amount_money" id="amount_money" onClick={(e) => e.target.value = ''} /></span></div>
                        <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Saldo pendiente </span>{(Number(credit.amount) - Number(credit.Installment)).toFixed(2)}</p></div>
                        <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Total abonado </span>{credit.Installment}</p></div>
                        <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Fecha de inicio </span>{dayjs(credit.create_at, 'YYYY/MM/DD').format('DD [de] MMMM [de] YYYY')}</p></div>
                        <div className="info_credit_customer"><p className="data_credit"><span className="description_info">Fecha de ultimo cambio</span>{dayjs(credit.updated_at, 'YYYY/MM/DD').format('DD [de] MMMM [de] YYYY')}</p></div>
                        ...
                        <div className="button_container_save">
                            <button className="edit_data_btn" onClick={() => editCredit(credit.id, textareaRef)}>Guardar Cambios</button>
                        </div>
                    </>
                )}
            </div>}
            ...
            {selectModal === "Money" && <div className="money_credit_Container">
                <section className="boxPayment installment">
                    ...
                    <button className="installment_button" onClick={() => installmentCredit(credit.id, credit.id_customer)}>Confirmar</button>
                </section>
                <section className="boxPayment payout">
                    <span className="payout_span">Liquidar</span>
                    <div className="payout_container">
                        <span className="payout_total">total a liquidar: ${(Number(credit.amount) - Number(credit.Installment)).toFixed(2)}</span>
                    </div>
                    <button className="payout_button" onClick={() => payoutCredit(credit.id, credit.id_customer)}>Confirmar</button>
                </section>
            </div>}
                    ...
                    {installmentH.length === 0 && !loading?
                        ...
                        return (
                            ...
                            <li key={index} className="container_data_installment">
                                ...
                                <span className="data_credit_installment">${element.installment_amount}</span>
                                <span className="data_credit_installment">{element.pay_status}</span>
                            </li>
                        )
